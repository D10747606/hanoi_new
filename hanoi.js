...
let instructions = [];
// task of moving n from source pier to distination
//  and using temp as temporary storage
...
{
    let p = instructions[k];  console.log(p);
    if(p === null) return;
    let s = p[0];
    let d = p[1];
    ...
    if (q.length > 0)
    {
        ...
        x1 = disk1.style.left;
        y1 = disk1.style.top;
        y1 = (parseInt(y1.substrinq(0,y1.length-2)) - THICKNESS) + 'px';
    }
    q.push(topid);
